      ...
      {MeshGradientRenderer && (
        <div className={`transition-opacity duration-1000 ease-in-out ${showGradient ? 'opacity-70' : 'opacity-0'}`}>
          <MeshGradientRenderer
            ...
            speed={0.01}
          />
          <NoiseOverlay opacity={0.5} blendMode="color-burn" baseFrequency="0.35" />
        </div>
      )}
...
const Intro = () => {
  ...
  return (
    <div
      id="presentation"
      className="lg:px-14 lg:mt-10 mt-4 w-full flex flex-col items-start dark:text-rose-50"
    >
      ...
      <p className="lg:text-xl text-base font-semibold text-rose-700 dark:text-orange-200 mb-3 lg:w-3/5 leading-snug">
        AI & music researcher building foundation models for controllable retrieval and multimodal music understanding.
      </p>

      <p className="lg:text-sm text-xs dark:text-rose-50/65 text-black/55 lg:w-1/2 mb-10 leading-relaxed">
  ...
  <div className="lg:px-14 w-full flex flex-col dark:text-rose-50">
    <div className="flex lg:flex-row flex-col lg:gap-10 gap-10 items-start">
      ...
      <div className="lg:w-1/2 w-full flex flex-col">
        ...
        {/* Currently card */}
        <div className="rounded-2xl bg-white/35 dark:bg-black/25 backdrop-blur-sm shadow-lg p-5 mb-8">
          ...
          <p className="text-xs text-black/55 dark:text-white/45 leading-snug mb-4">
            Queen Mary University of London × Universal Music Group
          </p>
          <div className="h-px bg-black/8 dark:bg-white/10 mb-4" />
          <p className="text-xs font-semibold uppercase tracking-widest text-rose-500 dark:text-orange-400 mb-2.5">Research focus</p>
          <div className="flex flex-wrap gap-2 mb-4">
            {["Foundation models", "Music retrieval", "Multimodal understanding", "Self-supervised learning"].map(tag => (
              <span key={tag} className="px-2.5 py-1 rounded-full text-xs bg-rose-400/12 dark:bg-orange-400/10 text-rose-700 dark:text-orange-300 font-medium">
                {tag}
              </span>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-2 text-xs text-black/55 dark:text-white/45">
        ...
        {/* Skills — no border, just the scroll strip */}
        <p className="text-xs font-semibold uppercase tracking-widest text-rose-500 dark:text-orange-400 mb-3">Skills & Tools</p>
        <Infinitescroll />
      </div>
